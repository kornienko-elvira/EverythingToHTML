...
import { EmailIcon,  FacebookIcon,   LinkedinIcon,  PinterestIcon,  TelegramIcon,   TwitterIcon,   WhatsappIcon } from "react-share";
...
const JewelleryEditorComponent = () => {
  ...
  const [JewelryImages, setJewelryImages] = React.useState(initialimages);
  const [BackgroundImages, setBackgroundImages] = React.useState(initialimages);
  const [ResponseSize, setResponsizeSize] = React.useState({width:320, height:200});
  const maxNumber = 69;
  ...
  const handleShareTwitterClick = async e => {
      ...
      Result.json().then(tx => {
        let url = tx.url
        window.open('https://twitter.com/home?status='.concat(url), 'twitter-popup', 'height=600,width=600');
      })
    }
  ...
  const handleSharePinterestClick = async e => {
      ...
      Result.json().then(tx => {
        let url = tx.url
        window.open('https://pinterest.com/pin/create/button/?url='.concat(url), 'facebook-popup', 'height=600,width=600');
      })
  }
  ...
  const handleShareLinkedinClick = async e => {
      ...
      Result.json().then(tx => {
        let url = tx.url
        window.open('https://www.linkedin.com/shareArticle?mini=true&url='.concat(url).concat('&title=&summary=&source='), 'linkedin-popup', 'height=600,width=600');
      })
  }
  ...
  const handleShareMailClick = async e => {
      ...
      Result.json().then(tx => {
        let url = tx.url
        window.open('mailto:info@example.com?&subject=&body='.concat(url).concat('&title=&summary=&source='), 'linkedin-popup', 'height=600,width=600');
      })
  }

  const handleShareTelegramClick = async e => {
      //const formData = new FormData();
      //let dataURL = stageRef.current.getStage().toDataURL();
      //formData.append(0, dataURL);
      //let Result = await fetch(`${API_URL}/image-upload`, {method: 'POST',  body: formData });
      //Result.json().then(tx => {
        let url = "https://google.com";//tx.url
        window.open('tg://msg_url?url='.concat(url).concat('&title=&summary=&source='), 'telegram-popup', 'height=600,width=600');
      //})
  }

  const handleShareWhatsappClick = async e => {
    const formData = new FormData();
    let dataURL = stageRef.current.getStage().toDataURL();
    formData.append(0, dataURL);
    let Result = await fetch(`${API_URL}/image-upload`, {method: 'POST',  body: formData });
  ...
  function resize() {
    setResponsizeSize({width:window.innerWidth - 200, height:window.innerHeight - 200})
    console.log("XXX:", ResponseSize);
      selectShape(null);
  }
      ...
      <div style={{display:"flex", justifyContent:"center"}}>
        ...
        <div>
          ...
          <div >
            ...
            <button onClick={handleShareTelegramClick} id="mail" style={{border:"none", background:"none"}}><TelegramIcon  width="40" height="40" round={true}/></button>
            <button onClick={handleShareWhatsappClick} id="mail" style={{border:"none", background:"none"}}><WhatsappIcon  width="40" height="40" round={true}/></button>
          </div>
          ...
          <div
            onDrop={e => {
              ...
              // add image
              if(dragType.current === "Background") {
                setBackgroundImages(
                  BackgroundImages.concat([
                    {
                      ...stageRef.current.getPointerPosition(),
                      src: dragUrl.current,
                      id: BackgroundImages.length + 1000
                    }
                  ])
                );
              } else {
                setJewelryImages(
                  JewelryImages.concat([
                    {
                      ...stageRef.current.getPointerPosition(),
                      src: dragUrl.current,
                      id: JewelryImages.length + 100
                    }
                  ])
                );
              }
          ...
          >
            <Stage
              ...
              style={{ border: '1px solid #EEE', boxShadow:"0px 0px 20px 10px rgba(245,245,245,0.6)", width:"100%", height:"100%", margin:"auto"}}
              ref={stageRef}
            >
              <Layer>
                {BackgroundImages.map((image, i) => {
                  return <URLImage image={image} 
                    ...
                    onChange={newAttrs => {
                      const rects = BackgroundImages.slice();
                      rects[i] = newAttrs;
                      setBackgroundImages(rects);
                    }}
                  />;
                })}
              </Layer>
              ...
              <Layer>
                {JewelryImages.map((image, i) => {
                  return <URLImage image={image} 
                    ...
                    onChange={newAttrs => {
                      const rects = JewelryImages.slice();
                      rects[i] = newAttrs;
                      setJewelryImages(rects);
                    }}
                  />;
                })}
              </Layer>
